...
export default function App() {
  useEffect(() => {
    const applySettings = (settings: any) => {
      ...
      if (settings.sidebarBtnActive)
        root.style.setProperty(
          "--sidebar-btn-active",
          settings.sidebarBtnActive,
        );
      ...
      if (settings.tablePadding)
        root.style.setProperty(
          "--table-padding",
          settings.tablePadding,
        );
      if (settings.radius)
        root.style.setProperty("--radius", settings.radius);
      if (settings.titleAlign) {
        const [flexAlign, textAlign] =
          settings.titleAlign.split("|");
    ...
    const handleStorageChange = (e: StorageEvent) => {
      if (
        e.key === "PayrollApp_UiSettings_small" &&
        e.newValue
      ) {
        try {
    ...
    const loadAndApply = async () => {
      ...
      try {
        const fullSaved = await localforage.getItem(
          "PayrollApp_UiSettings",
        );
        if (fullSaved) {
