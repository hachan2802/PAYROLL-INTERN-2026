...
export function MasterAE() {
  ...
  const [activeTab, setActiveTab] = useState<
    'Sheet1_AE' | 'Bank_North_AE' | 'Hold_AE' | 'SoSanh_AE' | 'CustomReport'
  >('Sheet1_AE');
    ...
    () => [
      {
        ...
        onClick: (selectedRows: any[]) => {
          updateAppData((prev) => {
            const currentData = prev[activeTab].data;
            ...
            return {
              ...prev,
              [activeTab]: { ...prev[activeTab], data: reindexedData },
            };
          });
  ...
  const processAEData = async () => {
    ...
    try {
        ...
        for (const sheetName of relevantSheets) {
          ...
          if (nameUpper.includes('BANK') || nameUpper.includes('NGÂN HÀNG')) {
            ...
            if (headerRowIndex !== -1) {
              ...
              const colMap = findColumnMapping(
                h,
                [
                  'STT',
                  'ID NUMBER',
              ...
              for (let r = headerRowIndex + 1; r < rows.length; r++) {
                const row = rows[r];
                const totalVal =
                  colMap['TOTAL PAYMENT'] !== -1
                    ? parseMoneyToNumber(row[colMap['TOTAL PAYMENT']])
                    : 0;
                const nameVal =
                  colMap['FULL NAME'] !== -1
                    ? String(row[colMap['FULL NAME']]).trim()
                    : '';

                if (totalVal === 0 && !nameVal) continue;

                const acc =
                  colMap['BANK ACCOUNT NUMBER'] !== -1
                    ? String(row[colMap['BANK ACCOUNT NUMBER']]).replace(
                        /\s/g,
                        ''
                      )
                    : '';
                ...
                bankData.push({
                  ...
                  'ID Number':
                    colMap['ID NUMBER'] !== -1
                      ? String(row[colMap['ID NUMBER']]).trim()
                      : '',
                  'Full name': nameVal,
                  ...
                  'LOẠI CK': type,
                  'Payment details':
                    colMap['PAYMENT DETAILS'] !== -1
                      ? String(row[colMap['PAYMENT DETAILS']]).trim()
                      : '',
                  Bank: item.bank || '',
                  Tháng: item.month || '',
                });
            ...
            if (headerRowIndex !== -1) {
              ...
              for (let r = headerRowIndex + 1; r < rows.length; r++) {
                ...
                holdData.push({
                  ...
                  'Bank Account Number':
                    colMap['BANK ACCOUNT NUMBER'] !== -1
                      ? String(row[colMap['BANK ACCOUNT NUMBER']]).trim()
                      : '',
                  'TAX CODE':
                    colMap['TAX CODE'] !== -1
                      ? String(row[colMap['TAX CODE']]).trim()
                      : '',
                  'Contract No':
                    colMap['CONTRACT NO'] !== -1
                      ? String(row[colMap['CONTRACT NO']]).trim()
                      : '',
                  'TOTAL PAYMENT': totalVal,
                  'CENTER NOTE':
                    colMap['CENTER NOTE'] !== -1
                      ? String(row[colMap['CENTER NOTE']]).trim()
                      : '',
          ...
          if (nameUpper.includes('SHEET 1') || nameUpper.includes('SHEET1')) {
            ...
            for (let r = 0; r < Math.min(30, rows.length); r++) {
              ...
              if (rowStr.includes('FULL NAME') || rowStr.includes('HỌ VÀ TÊN'))
                matchCount++;
              if (rowStr.includes('ID NUMBER') || rowStr.includes('MÃ NV'))
                matchCount++;
              if (
            ...
            if (headerRowIndex !== -1) {
              ...
              const centerColIndex = h.findIndex((x) => {
                const v = String(x).trim().toUpperCase();
                const mappedCenter = item.columnMapping?.['Center']
                  ?.toUpperCase()
                  .trim();
                if (mappedCenter && v === mappedCenter) return true;
                return (
                  v === 'CENTER' ||
                  v.includes('COST CENTER') ||
                  v.includes('TRUNG TÂM') ||
                  v.includes('AE CODE') ||
                  v === 'AE' ||
                  v.includes('MÃ AE') ||
                  v === 'L07'
      ...
      const finalSheet1 = deduplicate(sheet1Data, [
        'ID Number',
        'L07',
        'TOTAL PAYMENT',
      ]);
      const finalBank = deduplicate(bankData, [
        'ID Number',
        'Bank Account Number',
        'TOTAL PAYMENT',
      ]);
      ...
      toast.success(
        `Xử lý xong: ${finalSheet1.length} Sheet1, ${finalBank.length} Bank, ${finalHold.length} Hold.`
      );
  ...
  const reMapAECodes = () => {
    ...
    updateAppData((prev) => {
      const newData = prev.Sheet1_AE.data.map((row) => {
        const rawCenterVal =
          row['_rawAE'] ||
          (row['L07'] ? row['L07'].replace(' (Chưa Map)', '') : '');
        if (rawCenterVal) {
          const rawCenterKey = rawCenterVal.toLowerCase();
          if (aeMap[rawCenterKey]) {
            count++;
  ...
  const addCustomRow = () => {
    updateAppData((prev) => {
      ...
      prev.CustomReport.headers.forEach((h) => {
        if (h !== 'STT') newRow[h] = '';
      });
      return {
        ...prev,
        CustomReport: {
          ...prev.CustomReport,
          data: [newRow, ...prev.CustomReport.data],
        },
      };
    });
  ...
  const importL07ToCustomRow = (l07: string) => {
    ...
    const totalPayment = sameL07Rows.reduce(
      (sum, r) => sum + parseMoneyToNumber(r['TOTAL PAYMENT']),
      0
    );
    ...
    updateAppData((prev) => {
      ...
      return {
        ...
        CustomReport: {
          ...prev.CustomReport,
          data: [...prev.CustomReport.data, newRow],
        },
      };
    });
  ...
  return (
    ...
    >
      ...
      {/* Main Content Card */}
      <div className="bg-white soft-card flex-1 flex flex-col min-h-0 relative z-10 w-full max-w-[1240px] overflow-hidden">
        <div className="absolute inset-0 striped-pattern opacity-[0.05] pointer-events-none" />
        {/* Integrated Header & Controls */}
        <div
          className="px-8 py-6 flex flex-col xl:flex-row items-center justify-between gap-6 border-b border-border bg-muted/10 shrink-0 relative"
        >
          <div className="absolute inset-0 pattern-dots opacity-[0.05] pointer-events-none" />
          <div className="flex items-center gap-5 relative z-10 mr-auto">
            ...
            <div>
              <h2 className="text-2xl md:text-3xl font-serif text-foreground tracking-tight leading-tight">
                Dữ liệu <span className="italic font-script text-primary text-3xl md:text-4xl lowercase">Master AE</span>
              </h2>
              <p className="text-[0.625rem] font-bold text-muted-foreground uppercase tracking-[0.2em] mt-1 whitespace-nowrap">
                MANAGEMENT & RECONCILIATION •{' '}
                {currentData.data.length} RECORDS
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 relative z-10 shrink-0">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="h-10 px-6 rounded-xl bg-primary text-white shadow-md flex items-center gap-2 transition-all active:scale-95 disabled:opacity-50"
              >
                {isProcessing ? <Loader2 className="w-4 h-4 animate-spin" /> : <RefreshCw className="w-4 h-4" />}
                <span className="text-[0.65rem] font-bold tracking-widest uppercase whitespace-nowrap">{isProcessing ? 'Đang xử lý...' : 'Xử lý ngay'}</span>
              </button>
              <DropdownMenu>
                <Tooltip>
                ...
                >
                  ...
                  {tabs.map((tab) => (
                    <DropdownMenuItem
                      ...
                      className={`cursor-pointer font-bold uppercase text-[0.6875rem] gap-3 p-3 rounded-xl transition-all ${activeTab === tab.id ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'hover:bg-primary/5'}`}
                    >
              ...
              <DropdownMenu>
                ...
                >
                  ...
                  <div className="max-h-[200px] overflow-auto custom-scrollbar py-1 px-1">
                    {columns.map((col: any) => (
                      <DropdownMenuItem
                        ...
                        className="flex items-center gap-3 px-3 py-2 rounded-xl cursor-pointer hover:bg-primary/5 transition-colors group"
                      >
                        <div
                        ...
                        >
                          {!tableRef.current?.hiddenColumns?.has(col.key) && (
                            <CheckSquare className="w-3 h-3 text-white" />
                          )}
                        </div>
                        <span
                          className={`text-[0.625rem] font-bold uppercase tracking-wider ${!tableRef.current?.hiddenColumns?.has(col.key) ? 'text-primary' : 'text-primary/30'}`}
                        >
                          {col.label}
                        </span>
                      </DropdownMenuItem>
                  ...
                  >
                    ...
                    <span>
                      Xóa dữ liệu {tabs.find((t) => t.id === activeTab)?.label}
                    </span>
                  </DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
        ...
        {/* Content */}
        <div className="flex-1 min-h-0 flex flex-col w-full relative z-10 overflow-hidden rounded-b-[2rem]">
          {currentData.data.length === 0 ? (
            <div className="flex-1 flex flex-col items-center justify-center text-primary/10 p-12">
              ...
              <p className="text-[0.625rem] font-bold uppercase opacity-40 tracking-widest mt-2">
                Vui lòng cấu hình file AE và nhấn "Xử Lý Dữ Liệu AE".
              </p>
              <Button
                onClick={processAEData}
                disabled={isProcessing}
                className="mt-8 bg-primary text-white font-bold uppercase text-[0.625rem] tracking-widest px-10 py-4 h-12 rounded-2xl shadow-hard shadow-primary/20 hover:bg-primary/90 transition-all"
              >
                {isProcessing ? (
      ...
      <ConfirmDialog
        ...
        onConfirm={() => {
          ...
          toast.success(
            `Đã xóa dữ liệu tab ${tabs.find((t) => t.id === activeTab)?.label}`
          );
      ...
      <Dialog open={showImportModal} onOpenChange={setShowImportModal}>
        <DialogContent className="sm:max-w-2xl border border-primary/10 shadow-2xl bg-white rounded-[2.5rem] p-0 overflow-hidden">
          <DialogHeader className="p-8 bg-primary/5 border-b border-primary/10">
            <DialogTitle className="font-bold uppercase tracking-widest text-primary flex items-center gap-3 text-sm">
              <div className="w-10 h-10 bg-primary/10 rounded-xl flex items-center justify-center">
                <Import className="w-5 h-5" />
              </div>
              Chọn AE từ Master Data
            </DialogTitle>
            <DialogDescription className="font-bold text-primary/40 text-[0.625rem] uppercase tracking-widest mt-2 ml-13">
              Click vào dòng để thêm tổng tiền theo Trung tâm vào Báo Cáo Tùy
              Chỉnh
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="p-8 space-y-6">
            <div className="relative group">
              <Search className="w-4 h-4 absolute left-5 top-1/2 -translate-y-1/2 text-primary/30 group-focus-within:text-primary transition-colors" />
              <input
                type="text"
                placeholder="Tìm theo Tên, ID, Center..."
                value={importSearch}
                onChange={(e) => setImportSearch(e.target.value)}
                className="w-full pl-12 pr-6 py-4 bg-primary/5 border border-primary/10 rounded-2xl text-xs font-bold uppercase focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all shadow-inner"
              />
            </div>

            <div className="max-h-[400px] overflow-y-auto custom-scrollbar space-y-4 pr-2">
              {filteredImportList.length === 0 ? (
                <div className="text-center py-16 text-primary/20 font-bold uppercase text-[0.625rem] tracking-widest">
                ...
                filteredImportList.map((item, idx) => (
                  <motion.div
                    ...
                    className="p-5 bg-white border border-primary/10 rounded-[1.5rem] hover:border-primary hover:bg-primary/5 cursor-pointer transition-all group flex items-center justify-between shadow-sm hover:shadow-md"
                  >
                    <div>
                      <div className="font-bold text-foreground text-sm uppercase tracking-tight">
                        Trung tâm: {item.l07}
                      </div>
                      <div className="text-[0.625rem] font-bold text-foreground/40 flex gap-6 mt-3 uppercase tracking-widest">
                        <span className="flex items-center gap-2">
                          <Users className="w-3.5 h-3.5" /> AE: {item.count}
                        </span>
                        <span className="text-primary flex items-center gap-2">
                          <Landmark className="w-3.5 h-3.5" /> TỔNG:{' '}
                          {item.total.toLocaleString('vi-VN')} VND
                        </span>
                      </div>
                    </div>
                    <div className="w-12 h-12 bg-primary/10 rounded-2xl flex items-center justify-center text-primary opacity-0 group-hover:opacity-100 transition-all transform translate-x-2 group-hover:translate-x-0 shadow-sm">
          ...
          <DialogFooter className="p-8 bg-primary/5 border-t border-primary/10">
            <Button
              variant="outline"
              onClick={() => setShowImportModal(false)}
              className="border-primary/10 bg-white font-bold uppercase text-[0.625rem] tracking-widest px-8 py-3 h-11 rounded-xl hover:bg-primary/5 transition-all shadow-sm"
            >
              Đóng
            </Button>
          </DialogFooter>
